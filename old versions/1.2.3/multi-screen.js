		...
		if ($.multi_screen.lock_navigation === false) {
			...
			if (target !== false && target[0] !== current[0]) {
				...
				var exit_str = link.attr('data-ms-exit'),
					...
					max_height = Math.max.apply(Math, [current.outerHeight(), target.outerHeight(), $(window).height()]),
...
$.multi_screen.get_target_css = function (input_str, height, width) {
	...
	var post_shared = {position: 'absolute', zIndex: '2'},
		animate_shared = {left: '0', top: '0'},
		x_movement = (width + 200) + 'px',
		y_movement = (height + 200) + 'px';

	if (input_str == 'fade') {
	
		return {animate_css: {opacity: '1'}, pre_css: {display: 'block', opacity: '0', left: '0', top: '0'}, post_css: post_shared};
	
	} else if (input_str == 'east') {
	
		return {animate_css: animate_shared, pre_css: {display: 'block', left: x_movement, top: '0'}, post_css: post_shared};
		
	} else if (input_str == 'south') {
	
		return {animate_css: animate_shared, pre_css: {display: 'block', left: '0', top: y_movement}, post_css: post_shared};
		...
		return {animate_css: animate_shared, pre_css: {display: 'block', left: ('-' + x_movement), top: ('-' + y_movement)}, post_css: post_shared};
...
$.multi_screen.get_current_css = function (input_str, height, width) {
	...
	var post_shared = {display: 'none'},
		pre_shared = {position: 'fixed', zIndex: '1'},
		x_movement = (width + 200) + 'px',
		y_movement = (height + 200) + 'px';
	...
	if (input_str == 'fade') {
		...
		return {animate_css: {left: x_movement}, pre_css: pre_shared, post_css: post_shared};
		
	} else if (input_str == 'south') {
	
		return {animate_css: {top: y_movement}, pre_css: pre_shared, post_css: post_shared};
		...
		return {animate_css: {left: ('-' + x_movement), top: ('-' + y_movement)}, pre_css: pre_shared, post_css: post_shared};
		...
		return {animate_css: {top: y_movement, left: x_movement}, pre_css: pre_shared, post_css: post_shared};
